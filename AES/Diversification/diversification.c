...
uchar W[240];                  // La clef longue a une longueur maximale de (14+1)*16=240 octets
...
void calcule_la_clef_courte(char * clef){
  printf("calcule_la_clef_courte %s\n", clef);
  for(int i = 0; i < longueur_de_la_clef; i = i+2){
    char octet[2] = {clef[i],clef[i+1]};

    K[i] = octet;
  }
}
uchar* RotWord(uchar *word){
  uchar rotWord[4];
  for (int i = 0; i < 4; ++i){
    rotWord[i] = word[(i+3)%4];
  }
  return rotWord;
}

uchar* SubWord(uchar *word){
  uchar subWord[4];
  for (int i = 0; i < 4; ++i){
    int cell = (int)strtoul(word[i], NULL, 0);
    subWord[i] = word[cell];
  }
  return subWord;
}
...
{
  printf("calcule_la_clef_etendue\n");
  ...
  for(int i = Nk; i<longueur_de_la_clef_etendue; i++){
    uchar *tmp = (uchar*) malloc(sizeof(uchar)*4);
    for(int j = 4; j > 0; --j){
      tmp[j] = W[i-j];
    }
    printf("précedént récupérer\n");
...
int main(int argc, char* argv[])
{
  ...
  calcule_la_clef_etendue(K);                    // Fonction à écrire pour compléter le programme
