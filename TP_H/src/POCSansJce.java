...
public class POCSansJce {
	...
	public static byte[] SBox = { 0x63, (byte) 0x7C, (byte) 0x77, (byte) 0x7B, (byte) 0xF2, (byte) 0x6B, (byte) 0x6F,
	...
	static int Nk, Nr;
	private static byte[] encodage; // C�est le tableau � encoder
	/*private static byte[] decodage = new byte[LG_MAX]; // Ce sera le tableau d�cod�
	private static BigInteger code, codeChiffre;*/
	//private static OAEP oaep = new OAEP();
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		System.out.println("Clef utilisee: 0x" + toHex(k));
		System.out.println("Vecteur d'initialisation: 0x" + toHex(iv));

		// Exposant e de la clef publique
		BigInteger e = new BigInteger("44bb1ff6c2b674798e09075609b7883497ae2e2d7b06861ef"
				+ "9850e26d1456280523319021062c8743544877923fe65f8" + "5111792a98e4b887de8ffd13aef18ff7f6f736c821cfdad"
				+ "98af051e7caaa575d30b54ed9a6ee901bb0ffc17e25d444" + "f8bfc5922325ee2ef94bd4ee15bede2ea12eb623ad507d6"
				+ "b246a1f0c3cc419f155", 16);
		// Module n de la clef publique
		BigInteger n = new BigInteger("94f28651e58a75781cfe69900174b86f855f092f09e3da2ad86b4ed964a84917e5ec60"
				...
				+ "c515097a3c556bf86f2048a3c159fccfee6d916d38f7f23f21", 16);

		//encodage = k;

		// Bourrage pkcs1 de la clef
		byte[] messageBourre = OAEP.blocMM(k);
		// Encodage
		BigInteger m = new BigInteger(1, messageBourre);
		BigInteger chiffre = m.modPow(e, n);

		System.out.println("Le message bourre = " + toHex(messageBourre));
		...
		etendreClefCourte();
		//byte[] aeschiffre = new byte[2048];
		byte[] bloc = new byte[LG_MAX];
		Aes aes = new Aes();
		aes.setIv(iv);
		encodage = pkcs5Padding(encodage);
		for (int i = 0; i < Nr+1; ++i){
			System.arraycopy(encodage, i*(Nr+1), bloc, 0, LG_MAX);
			aes.setState(bloc);
			aes.useCBC();
			aes.chiffrer();
			bloc = aes.getState();
			aes.setIv(bloc);
			System.arraycopy(bloc, 0, encodage, i*(Nr+1)+1, LG_MAX);
		}
		
		try {
			FileOutputStream fos = new FileOutputStream("crypt-butokuden-sans-jce.jpg");
			fos.write(messageBourre);
			fos.write(iv);
			fos.write(encodage);
			fos.close();
	...
	public static byte[] pkcs5Padding(byte[] file) {
		int bytesLeft = 16 - file.length%16;
		// Fonctionne car les valeurs attendues vont de 0 à 15.
		byte padValue = Byte.parseByte(Integer.toString(bytesLeft));
		byte[] padFile = new byte[file.length+bytesLeft];
		System.arraycopy(file, 0, padFile, 0, file.length);
		for(int i = file.length; i < padFile.length; ++i) {
			padFile[i] = padValue;
		}
		return padFile;
	}
