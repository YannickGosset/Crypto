...
public class Aes {
	...
	byte iv[] = new byte[longueur_de_la_clef];
	...
	public void setState(byte[] state) {
		if(state.length == State.length)
			...
			System.err.println("Mauvaise taille du nouveau bloc d'état.");
	}
	...
	public void setIv(byte[] iv) {
		if(this.iv.length == iv.length)
			this.iv = iv;
		else
			System.err.println("Mauvaise taille du nouveau vecteur d'initialisation.");
	}
	...
	public void useCBC() {
		for(int i = 0; i < State.length; ++ i) {
			State[i] =  Byte.parseByte(Integer.toString(State[i] ^ iv[i])); 
		}
	}
